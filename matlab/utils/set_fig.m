function [fig,ax] = set_fig(fig,varargin)
%
% Set figure
%
% Usage:
...
addParameter(ps,'axes_info','');                     % set axes margin 
addParameter(ps,'REMOVE_TICK',0);                    % remove axis ticks
parse(ps,varargin{:});
pos             = ps.Results.pos;
...
axes_info       = ps.Results.axes_info;
REMOVE_TICK     = ps.Results.REMOVE_TICK;
...
if h{fig.Number}.first_flag || ~ishandle(h{fig.Number}.fig)
    ...
    % Set margin
    if ~isempty(axes_info)
        ax = axes('Parent',fig,'Position',axes_info);
    end
    h{fig.Number}.ax = ax;
